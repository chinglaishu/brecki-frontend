...
export const SettingStack: FC<PageProps> = ({navigation}) => {
  ...
  const getContent = (contextObj: ContextObj) => {
    ...
    const {language} = user;
    return (
      <Stack.Navigator initialRouteName={SCREEN.SETTING} screenOptions={{animationEnabled: true, header: (props) => null }}
        detachInactiveScreens={false}>
        <Stack.Screen name={SCREEN.SETTING} component={Setting} initialParams={{changeStatusModal, userId: user.id}}/>
        <Stack.Screen name={SCREEN.ACCOUNT} component={Account} initialParams={{changeStatusModal}}/>
      </Stack.Navigator>
    );
  };
