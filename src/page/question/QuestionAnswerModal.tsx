...
import { FONT_NORMAL, SCREEN, STATUS_TYPE } from "../../constant/constant";
import { getRequestToAnswerQuestions, submitQuestion } from "../../request/question";
...
import { checkIfRequestError, checkIsSwipe, makeRequestWithStatus } from "../../utils/utilFunction";
import { SelectQuestionNumModal } from "./SelectQuestionNumModal";
import { Question, QuestionChoiceRecord, SubmitQuestionRecord } from "./type";
...
export const QuestionAnswerModal: FC<QuestionAnswerModalProps> = ({questions, slideIndex, setSlideIndex, isFocusQuestion,
  setIsFocusQuestion, navigation, onAllSubmit}) => {
  ...
  const [questionChoiceRecords, setQuestionChoiceRecords] = useState(getDefaultQuestionChoiceRecord(questions));
  ...
  const onSubmit = async (changeStatusModal: any) => {
  ...
  const getContent = (contextObj: ContextObj) => {
    ...
    return (
      <>
        ...
        {/* <KeyboardAvoidingView style={{paddingBottom: 0}} behavior={"padding"}> */}
          <GestureRecognizer
            style={{}}
              ...
              <ContainerView style={{backgroundColor: TRANSPARENT}}>
                ...
                <View style={{margin: wp(5), marginBottom: hp(4),
                  ...
                  <Text style={{color: theme.onPrimary, fontSize: hp(2), marginTop: hp(1),
                    fontFamily: FONT_NORMAL}}>{`A: ${currentAnswer}${typingString}`}</Text>
                  <RowView style={{position: "absolute", right: wp(2), bottom: wp(2), flex: 1, alignItems: "flex-end", justifyContent: "flex-end"}}>
                    <ButtonTouchable style={{backgroundColor: "#00000000", borderColor: theme.warning, elevation: 0,
                      marginRight: wp(2)}}
                    ...
                    <ButtonTouchable style={{backgroundColor: "#00000000", borderColor: buttonColor, elevation: 0}}
          ...
          {/* add painting modal here? seems need to be in the modal */}
