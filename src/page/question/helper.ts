...
import { Question, QuestionChoice, QuestionChoiceRecord } from "./type";
...
export const uploadBase64InQuestionChoiceRecords = async (questionChoiceRecords: QuestionChoiceRecord[]) => {
  return await Promise.all(questionChoiceRecords.map(async (questionChoiceRecord) => {
    ...
    const uploadResult = await uploadImage(base64, "", IMAGE_S3_DIRECTORY.QUESTION_DRAWING);
...
export const getDefaultQuestionChoiceRecord = (questions: Question[]) => {
...
export const checkErrorWhenGoToNextQuestion = (questionChoiceRecord: QuestionChoiceRecord) => {
  ...
  const {choiceId, isChoosingContent, isChoosingImage, base64, content} = questionChoiceRecord;
  if (!choiceId) {return true; }
  if (isChoosingContent && (!content || content.length === 0)) {return true; }
  if (isChoosingImage && (!base64 || base64.length === 0)) {return true; }
  return false;
};
